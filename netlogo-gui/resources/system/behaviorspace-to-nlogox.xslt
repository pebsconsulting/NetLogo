...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="experiment">
    <experiment name="{@name}">
      ...
      <parameterSet>
        <cartesianProduct>
          <xsl:if test="@repetitionsPerCombo != 1">
            <xsl:attribute name="repetitionsPerCombo">
              <xsl:value-of select="@repetitions" />
            </xsl:attribute>
          </xsl:if>
          <xsl:if test="@sequentialRunOrder = 'false'">
            <xsl:attribute name="sequentialRunOrder">true</xsl:attribute>
          </xsl:if>
          <xsl:apply-templates select="enumeratedValueSet" />
          <xsl:apply-templates select="steppedValueSet" />
        </cartesianProduct>
      </parameterSet>
    </experiment>
  </xsl:template>
